...
BEGIN
  ...
  -- Fallback: all-time top innovator
  IF top_innovator.full_name IS NULL THEN
    SELECT p.full_name, COUNT(i.id) AS cnt
    INTO top_innovator
    FROM public.innovations i
    JOIN public.profiles p ON p.id = i.user_id
  ...
  -- Fallback: all-time top learner
  IF top_learner.full_name IS NULL OR COALESCE(top_learner.total_seconds, 0) = 0 THEN
    SELECT p.full_name,
           COALESCE(SUM(ve.watch_time_seconds), 0) AS total_seconds
    INTO top_learner
    FROM public.video_engagement ve
